...
def handle_overly_permissive_iam(finding_info):
    try:
        ...
        for binding in policy.bindings:
            ...
            # Filter members
            original_count = len(binding.members)
            safe_members = [m for m in binding.members if not m.startswith("serviceAccount:")]
            
            if len(safe_members) < original_count:
                removed_count += original_count - len(safe_members)
                modified = True
                print(f"[GCP GUARD] Removing {binding.role} from {original_count - len(safe_members)} service account(s)")
                
                # Clear and rebuild members list
                del binding.members[:]
                binding.members.extend(safe_members)
        
        if modified:
            # Use SetIamPolicyRequest
            ...
            return {
                ...
                "message": f"Removed {removed_count} overly permissive IAM binding(s)"
            }
        else:
            return {
                "status": "no_action",
